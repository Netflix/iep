 ...
 */
public class ModifiedJsonObjectDecoder extends ByteToMessageDecoder {
    ...
    public ModifiedJsonObjectDecoder() {
        // 1 MB
        this(1024 * 1024);
    }

    public ModifiedJsonObjectDecoder(int maxObjectLength) {
        this(maxObjectLength, false);
    }

    public ModifiedJsonObjectDecoder(boolean streamArrayElements) {
     ...
     *
     */
    public ModifiedJsonObjectDecoder(int maxObjectLength, boolean streamArrayElements) {
...
//System.err.println("decode: " + in + " [" + idx + "," + openBraces + "," + insideString + "]");
        if (in.readerIndex() > 0) {
          if (in.readerIndex() > this.idx)
              throw new IllegalStateException(
                  "expected reader index [" + in.readerIndex() + "] to be > idx [" + idx + "]"
              );
          int idx = this.idx - in.readerIndex();
//System.err.println("setting this.idx=0");
          this.idx = idx;
          return;
        }
        if (state == ST_CORRUPTED) {
//System.err.println("skipping corrupt " + in.readableBytes() + " bytes");
...
//System.err.println("skipping maxlenth " + in.readableBytes() + " bytes");
            in.skipBytes(in.readableBytes());
            reset();
...
//System.err.println("loop: " + " [" + idx + "," + wrtIdx + "," + openBraces + "," + insideString + "]");
        for (/* use current idx */; idx < wrtIdx; idx++) {
            byte c = in.getByte(idx);
//System.err.printf("byte: %c%n", c);
...
//System.err.println("setting this.idx=" + idx);
            this.idx = idx;
        }
    }
